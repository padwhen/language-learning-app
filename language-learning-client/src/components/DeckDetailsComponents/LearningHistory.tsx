                    ...
                    {history && history.length > 0 ? 
                        <ul className="space-y-2">
                            {history.map((item, index) => (
                                <li key={index}>
                                    <Link to={`/view-decks/${deckId}/learning-report/${item.id}`} className="font-bold underline text-blue-500 hover:text-blue-700">{item.randomName} {format(new Date(item.date), 'dd.MM.yyyy')}</Link>: {item.quizType === 'learn' ? 'Learned' : 'Reviewed'} {item.correctAnswers ? item.correctAnswers : 'Error'} out of {item.cardsStudied} cards. 
                                </li>
                            ))}
                        </ul>
                    : (<div>
